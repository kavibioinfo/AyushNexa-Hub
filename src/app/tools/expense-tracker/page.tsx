...
export default function ExpenseTracker() {
  ...
  useEffect(() => {
    ...
    if (savedPremium === "true") setIsPremiumUnlocked(true);
    ...
    if (savedOpening !== null) setOpeningBalance(Number(savedOpening));
    else setOpeningBalance(0);
  }, []);

  useEffect(() => {
    if (isMounted) localStorage.setItem("ayushnexa_ledger_data", JSON.stringify(transactions));
  }, [transactions, isMounted]);

  useEffect(() => {
    if (isMounted) localStorage.setItem("ayushnexa_opening_balance", openingBalance.toString());
  }, [openingBalance, isMounted]);

  useEffect(() => {
    if (isMounted) localStorage.setItem("expense_tracker_premium", isPremiumUnlocked.toString());
  ...
  const totalExpense = currentMonthTx.filter((t) => t.type === "expense").reduce((a, t) => a + t.amount, 0);
  ...
  const pieData = Object.entries(expenseByCategory).map(([cat, amt]) => ({ name: cat, value: amt }));
  ...
  const monthlyTrend = MONTHS_LIST.map((month) => {
    ...
    return { month: month.label, उत्पन्न: income, खर्च: expense };
  });
  ...
  const handleDelete = (id: number) => setTransactions(transactions.filter((t) => t.id !== id));
  ...
  if (!isMounted) return <div className="min-h-screen bg-[#FFF8F0]" />;
  ...
  return (
    <div className="min-h-screen bg-[#FFF8F0] text-[#1A1A1A] antialiased print:bg-white pb-12">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        {/* Header */}
        <div className="bg-[#FF6B35] text-white p-5 sm:p-6 rounded-2xl shadow-md flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
          ...
          <div className="flex gap-3 w-full md:w-auto">
            <button
              ...
              className="flex-1 md:flex-none px-5 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 text-white font-bold text-sm border border-white/20 transition-all"
            >
              🧹 डेटा साफ करा
            </button>
            <button
              onClick={() => (isPremiumUnlocked ? window.print() : setShowPaywall(true))}
              className="flex-1 md:flex-none px-5 py-2.5 rounded-xl bg-[#10B981] text-white font-bold text-sm shadow hover:bg-[#10B981]/90 transition-all"
            >
        ...
        {/* Month selector */}
        <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-3 mb-6 scrollbar-none print:hidden">
          {MONTHS_LIST.map((m) => (
            <button
              key={m.key}
              onClick={() => setSelectedMonth(m.key)}
              className={`px-4 sm:px-6 py-2.5 sm:py-3 rounded-full text-sm sm:text-base font-bold transition-all shrink-0 border ${
                selectedMonth === m.key
        ...
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 mb-8">
          ...
          <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-xl text-center flex flex-col justify-center">
            <span className="text-xs font-black uppercase text-emerald-700">📈 एकूण जमा</span>
          ...
          <div className="bg-red-50 border border-red-100 p-4 rounded-xl text-center flex flex-col justify-center">
            <span className="text-xs font-black uppercase text-red-700">📉 एकूण खर्च</span>
            <h3 className="text-xl sm:text-2xl font-black text-red-800 mt-1">
          ...
          >
            <span className="text-xs font-black uppercase opacity-80">👛 निव्वळ शिल्लक</span>
            <h3 className="text-xl sm:text-2xl font-black mt-1">₹{netBalance.toLocaleString("en-IN")}</h3>
          </div>
        </div>
        ...
        <div className="grid gap-6 grid-cols-1 lg:grid-cols-3">
          ...
          <div className="bg-white rounded-2xl p-5 border border-[#E2E8F0] shadow-sm h-fit print:hidden">
            <h3 className="text-lg sm:text-xl font-black text-[#FF6B35] border-b border-[#E2E8F0] pb-2 mb-4">
              ✏️ नवीन नोंद जोडा
            </h3>
            <form onSubmit={handleAdd} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-black text-[#64748B] mb-1">कशासाठी खर्च/जमा केला?</label>
                <input
                  type="text"
                  placeholder="उदा. किराणा, भाजीपाला, दूध"
                  className="w-full rounded-lg border border-[#E2E8F0] p-3 text-sm outline-none focus:border-[#FF6B35]"
                  value={title}
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  <label className="block text-sm font-black text-[#64748B]">किती रुपये? (₹)</label>
              ...
              <div>
                <label className="block text-sm font-black text-[#64748B] mb-1">वर्ग निवडा</label>
                <select
                  className="w-full rounded-lg border border-[#E2E8F0] p-3 text-sm bg-white"
                  value={category}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl p-5 border border-[#E2E8F0] shadow-sm">
              <h3 className="text-lg sm:text-xl font-black text-[#1A1A1A] border-b border-[#E2E8F0] pb-2 mb-4 flex justify-between items-center flex-wrap gap-2">
                <span>📋 हिशोब वही ({MONTHS_LIST.find((m) => m.key === selectedMonth)?.label})</span>
                <span className="text-xs bg-gray-100 px-2 py-1 rounded-full text-gray-600 font-bold">
              ...
              <div className="space-y-3">
                {currentMonthTx.map((tx) => (
                  <div
                    key={tx.id}
                    className="flex justify-between items-center p-3 sm:p-4 border border-gray-100 rounded-xl bg-white shadow-sm"
                  >
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      <div
                        className={`h-10 w-10 sm:h-12 sm:w-12 rounded-full flex items-center justify-center text-base sm:text-xl shrink-0 ${
                      ...
                      <div className="min-w-0 flex-1">
                        <h4 className="text-sm sm:text-lg font-black text-[#1A1A1A] break-words">{tx.title}</h4>
                        <span className="text-xs sm:text-sm font-bold text-gray-500 bg-gray-50 px-2 py-0.5 rounded mt-0.5 inline-block">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <button
                        ...
                        className="h-8 w-8 sm:h-10 sm:w-10 bg-red-50 text-red-600 rounded-lg flex items-center justify-center text-sm print:hidden"
                      >
                        🗑️
                      </button>
                    </div>
            ...
            {/* Premium Analytics */}
            {isPremiumUnlocked && (
              <div className="bg-white rounded-2xl p-5 border border-[#E2E8F0] shadow-sm space-y-6">
                <h3 className="text-lg sm:text-xl font-black text-[#1A1A1A] border-b border-[#E2E8F0] pb-2 mb-4 flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                  <div className="bg-emerald-50 p-3 rounded-lg">
                    ...
                    <p className="text-gray-700 mt-1">
                      {pieData.length > 0 ? pieData.reduce((a, b) => (a.value > b.value ? a : b)).name : "अद्याप खर्च नाही"}
                    </p>
                  </div>
