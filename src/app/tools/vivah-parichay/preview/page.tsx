...
export default function BiodataPreviewWorkspace() {
  ...
  const handleThemeChange = (themeId: string) => {
    ...
    if (selectedTheme && !isPremiumUnlocked && themeId !== 'classic') {
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-5 sm:py-8 grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-start">
        ...
        <div className="lg:col-span-5 xl:col-span-4 space-y-5 sm:space-y-6 lg:sticky lg:top-24 no-print">
            ...
            {activeTab === 'themes' && (
              <div className="p-4 sm:p-6 space-y-3 sm:space-y-4 max-h-[380px] overflow-y-auto">
                {PREMIUM_THEMES.map((theme) => {
                  ...
                  const isLocked = theme.id !== 'classic' && !isPremiumUnlocked;
                  return (
                    <button key={theme.id} onClick={() => handleThemeChange(theme.id)} disabled={isLocked} className={`w-full text-left p-3 sm:p-4 rounded-xl sm:rounded-2xl border-2 transition-all flex items-center justify-between ${isActive ? 'border-amber-600 bg-amber-50/20 text-amber-900 ring-2 ring-amber-400/25' : 'border-zinc-200 bg-white hover:bg-zinc-50 text-slate-700'} ${isLocked ? 'opacity-50 cursor-not-allowed' : ''}`}>
                      ...
                      {isLocked && <span className="text-amber-600 text-[10px] sm:text-xs font-bold flex items-center gap-1"><Sparkles className="w-3 h-3" /> Unlock</span>}
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-6 border border-zinc-200 shadow-md space-y-4">
            <h4 className="font-extrabold text-sm sm:text-base text-slate-900 border-b pb-2 flex items-center gap-1.5"><Download className="w-4 h-4 text-emerald-600" /> डाऊनलोड आणि शेअर पर्याय</h4>
            <button onClick={handlePrint} className="w-full bg-amber-600 hover:bg-amber-700 text-white font-extrabold text-sm sm:text-base py-3.5 sm:py-4 px-4 rounded-xl flex items-center justify-center gap-2 shadow-md transition-all active:scale-95"><Download className="w-4 h-4 sm:w-5 sm:h-5" /> प्रिंटेबल A4 PDF / प्रिंट डाऊनलोड करा</button>
          ...
          <div className="bg-gradient-to-tr from-amber-50 to-amber-100/15 border-2 border-amber-200 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm space-y-3">
            ...
            <div className="flex items-center gap-3 sm:gap-4 bg-white/70 p-3 rounded-xl border border-amber-200">
              <div className="w-14 h-14 sm:w-16 sm:h-16 bg-zinc-200 shrink-0 rounded border overflow-hidden"><img src={`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=tel:${state.contact.mobile || '9876543210'}`} alt="Contact QR" className="w-full h-full object-cover" /></div>
              <div className="flex-1">
                ...
                {!isPremiumUnlocked && <button onClick={() => setShowPremiumUpgradeModal(true)} className="text-[10px] sm:text-[11px] text-amber-600 underline font-black block mt-1.5">थीम मध्ये समाविष्ट करा (Premium)</button>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-3 sm:p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl sm:rounded-3xl p-5 sm:p-8 max-w-lg w-full border border-zinc-200 shadow-2xl relative space-y-5 sm:space-y-6">
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-xs sm:text-sm text-slate-500 font-serif">फक्त ₹१५१ भरून १० थीम्स, फोटो आणि QR कोड मिळवा.</p>
            </div>
            <div className="border-t border-b border-dashed border-zinc-200 py-3 sm:py-4 space-y-2.5 sm:space-y-3">
              {['सर्व १० डिझाईनर थीम्स', 'फोटो अपलोडिंग व कस्टमायझेशन', 'WhatsApp QR कोड', 'अमर्यादित डाऊनलोड', 'कोणताही वॉटरमार्क नसेल'].map((benefit, idx) => (
                <div key={idx} className="flex items-center gap-2.5 text-xs sm:text-sm text-slate-700 font-bold"><span className="text-emerald-600 bg-emerald-50 rounded-full p-0.5">✓</span><span>{benefit}</span></div>
              ))}
            </div>
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                <span className="text-[10px] sm:text-xs text-zinc-400 block font-mono">Premium Plan</span>
                <div className="flex items-baseline gap-2"><span className="text-2xl sm:text-3xl font-black text-slate-900">₹१५१</span><span className="text-xs text-zinc-400 line-through">₹४९९</span></div>
              </div>
              <RazorpayButton productId="vivah_premium" amount={151} productName="Vivah Parichay Premium" label="सुरक्षित पेमेंट (Razorpay)" onSuccess={handlePaymentSuccess} userEmail={state.contact.mobile || ''} />
            </div>
